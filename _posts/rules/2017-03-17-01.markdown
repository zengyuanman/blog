---
layout: post
title:  "前端开发规范-javascript-初稿"
date:   2017-03-17
description: "前端开发规范-javascript-初稿"
categories: 
---
...
> 本文档基于百度前端javascript 规范文档修改，后续会根据实际使用情况进行规范，输出一套eslint相关配置，在浏览器端进行配置，对javascript代码输出进行格式化，同时在构建打包阶段，使用 node 等构建工具根据eslint配置进行相关去毛检查工作。

> 文档的主要作用是对代码编写进行规范，避免一些不相关的细节导致一些莫名其妙问题的出现。
>
> 另外一些是对 javascript的写法进行避免，相关参考[javascript 语言精粹](http://product.dangdang.com/22872884.html)
>
> 尽量在 IDE 或者sublime text等文本编辑器中将代码去毛检查常态化，自动化。主动适应，拥抱变化。
